// Admin leaderboard — same live design as technician leaderboard, admin-gated
...
export default async function AdminLeaderboardPage({
  ...
  const ranked = technicians
    .map((t) => ({ id: t.id, name: t.name, points: techMap[t.id]?.points ?? 0, tickets: techMap[t.id]?.tickets ?? 0 }))
      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Live rankings for {month ? MONTHS[month - 1] : "All months"} {year} · Auto-updates on ticket completion
          </p>
        </div>
        ...
        <div className="leaderboard-layout">
          <div className="leaderboard-chart flex flex-col gap-4">
                ...
                {second && (() => { const cfg = RANK_CONFIG[1]; return (
                  <div style={{ flex:"0 0 100px", display:"flex", flexDirection:"column", alignItems:"center" }}>
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"4px", marginBottom:"8px" }}>
                      ...
                      <div style={{ width:cfg.avatarSize,height:cfg.avatarSize,borderRadius:"50%",background:cfg.bg,border:`2px solid ${cfg.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:cfg.fontSize,fontWeight:700,color:"#fff" }}>{getInitials(second.name)}</div>
                ...
                {first && (() => { const cfg = RANK_CONFIG[0]; return (
                  <div style={{ flex:"0 0 120px", display:"flex", flexDirection:"column", alignItems:"center" }}>
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"4px", marginBottom:"8px" }}>
                      ...
                      <div style={{ width:cfg.avatarSize,height:cfg.avatarSize,borderRadius:"50%",background:cfg.bg,border:`2px solid ${cfg.border}`,boxShadow:`0 0 20px ${cfg.glow}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:cfg.fontSize,fontWeight:700,color:"#fff" }}>{getInitials(first.name)}</div>
                ...
                {third && (() => { const cfg = RANK_CONFIG[2]; return (
                  <div style={{ flex:"0 0 100px", display:"flex", flexDirection:"column", alignItems:"center" }}>
                    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"4px", marginBottom:"8px" }}>
                      ...
                      <div style={{ width:cfg.avatarSize,height:cfg.avatarSize,borderRadius:"50%",background:cfg.bg,border:`2px solid ${cfg.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:cfg.fontSize,fontWeight:700,color:"#fff" }}>{getInitials(third.name)}</div>
            ...
            {top5.length > 0 && (
              <div className="card" style={{ padding: "1.25rem" }}>
                <h3 style={{ fontSize:"0.875rem",fontWeight:700,color:"var(--text-secondary)",marginBottom:"1rem",textTransform:"uppercase",letterSpacing:"0.05em" }}>Top 5 — Points Breakdown</h3>
                  ...
                  {top5.map((t, i) => {
                    ...
                    return (
                      <div key={t.id} style={{ display:"flex",alignItems:"center",gap:"0.75rem" }}>
                        ...
                        <span style={{ width:"6rem",fontSize:"0.875rem",fontWeight:500,flexShrink:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{t.name}</span>
          ...
          <div className="leaderboard-list">
            <div style={{ padding:"0.75rem 0",borderBottom:"1px solid var(--border-light)",marginBottom:"0.5rem" }}>
              <h3 style={{ fontSize:"0.875rem",fontWeight:700,color:"var(--text-secondary)",textTransform:"uppercase",letterSpacing:"0.05em" }}>All Rankings</h3>
            </div>
              ...
              {ranked.map((t, i) => {
                ...
                return (
                  <div key={t.id} style={{ display:"flex",alignItems:"center",gap:"0.75rem",padding:"0.75rem 0.875rem",background:isTop3?`${medalColor}0d`:"var(--white)",border:`1.5px solid ${isTop3?`${medalColor}40`:"var(--border-light)"}`,borderRadius:"var(--radius-md)",animation:`fadeIn 0.4s ${i*0.05}s ease both` }}>
                    ...
                    <div style={{ width:"2rem",height:"2rem",borderRadius:"50%",flexShrink:0,background:isTop3?`${medalColor}30`:"var(--primary)",color:isTop3?medalColor:"white",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.6875rem",fontWeight:700,border:isTop3?`1.5px solid ${medalColor}60`:"none" }}>
                      {getInitials(t.name)}
                    </div>
                    <div style={{ flex:1,minWidth:0 }}>
                      <div style={{ fontWeight:600,fontSize:"0.875rem",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{t.name}</div>
                      <div style={{ fontSize:"0.75rem",color:"var(--text-muted)" }}>{t.tickets} ticket{t.tickets!==1?"s":""}</div>
                    </div>
